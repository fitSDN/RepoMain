 ...
 */
public class OFChannelHandler extends IdleStateAwareChannelHandler {
	...
	public static HashMap<String, Channel> westboundChannelSendingList = new HashMap<String, Channel>();
	public static HashMap<Channel, OFConnection> westboundConnSendingList = new HashMap<Channel, OFConnection>();
	
	public static HashMap<String, Channel> westboundChannelReceivingList = new HashMap<String, Channel>();
	public static HashMap<Channel, OFConnection> westboundConnReceivingList = new HashMap<Channel, OFConnection>();
	...
	public abstract class OFChannelState {
		...
		protected void logErrorDisconnect(OFErrorMsg error) {
			// Added for atomicity, do not disonnect connection!
			if (error.getErrType() != OFErrorType.BAD_ACTION) {	
				logError(error);
				channel.disconnect();
			}
		}
		...
		void processOFMessageEastWestbound(OFMessage m, String remoteHostAddress)
				throws IOException {
			// System.out.println("processOFMessageEastWestbound: " + m.getType());
			switch(m.getType()){
				case HELLO:
					break;
				case ERROR:
				...
				case ECHO_REQUEST:
					break;
				default:
					sendMessageToEastWestboundConnReceiving(m, remoteHostAddress);
					break;
			}
		}
	}
	...
	class WaitFeaturesReplyState extends OFChannelState{
		...
		void processOFFeaturesReply(OFFeaturesReply  m)
			...
			log.info("Connected switch parameters {}",getConnectionInfoString());
		}
		@Override
		void enterState() throws IOException {
			sendFeaturesRequest();
	...
	private final void sendMessageToEastWestboundConnReceiving(OFMessage m, String remoteHostAddress) {
		Channel wbiChannel = westboundChannelReceivingList.get(remoteHostAddress);
		OFConnection wbiConnection = westboundConnReceivingList.get(wbiChannel);
		wbiConnection.messageReceived(m);
	}

	// Four new getters for WBI
	public static Channel getWestboundChannelReceiving(String ctrlAddress) {
		return westboundChannelReceivingList.get(ctrlAddress);
	}
	
	public static Channel getWestboundChannelSending(String ctrlAddress) {
		return westboundChannelSendingList.get(ctrlAddress);
	}
	
	public static OFConnection getWestboundConnectionReceiving(Channel channel) {
		return westboundConnReceivingList.get(channel);
	}
	
	public static OFConnection getWestboundConnectionSending(Channel channel) {
		return westboundConnSendingList.get(channel);
	}
	// End
	...
	public void channelConnected(ChannelHandlerContext ctx,
		...
		if (localAddress.getPort() < 6666 || localAddress.getPort() > 6669) {
			log.debug("channelConnected on OFChannelHandler {}", String.format("%08x", System.identityHashCode(this)));
			counters.switchConnected.increment();
			log.info("New switch connection from {}",
					channel.getRemoteAddress());
			...
			// These are WBI channels
			
			System.out.println("New WBI channel conneted from: " + remoteAddress + " -> " + localAddress);
			
			if (localAddress.getPort() != 6666) {
				if (remoteAddress != null && remoteAddress.getPort() == 6666) {
					String remoteHostAddress = remoteAddress.getAddress().getHostAddress();
					
					if (westboundChannelSendingList.get(remoteHostAddress) == null) {
						Channel newWBIChannel = e.getChannel();
						OFConnection newWBIConnection = createNewWBIConnection(newWBIChannel);
						newWBIConnection.setListener(new IOFConnectionListener() {
						...
						System.out.println("Initialize WBI sending on local port: " + newWBIConnection.getLocalInetAddress());
						
						westboundChannelSendingList.put(remoteHostAddress, newWBIChannel);
						westboundConnSendingList.put(newWBIChannel, newWBIConnection);
					}
				}
			} else {
				//InetSocketAddress remoteAddress = (InetSocketAddress) channel.getRemoteAddress();
				...
				String remoteHostAddress = remoteAddress.getAddress().getHostAddress();
				if (westboundChannelReceivingList.get(remoteHostAddress) == null) {
					log.info("New ctrl connection from {}", channel.getRemoteAddress());
					
					System.out.println("Initialize WBI receiving!");
				    ...
				    newWBIConnection.setListener(new IOFConnectionListener() {
						...
						@Override
						public void messageReceived(IOFConnectionBackend connection, OFMessage m) {
							// System.out.println("WBI | OFMessage Received from " + connection.getRemoteInetAddress());
							
							if (OFSwitchManager.floodlightProvider != null) {
								// Message forwarded to the floodlightProvider for handling
								IOFSwitchBackend sw = createMock(IOFSwitchBackend.class);
								OFSwitchManager.floodlightProvider.handleMessage(sw, m, null);
							}
						}
				    ...
				    westboundChannelReceivingList.put(remoteHostAddress, newWBIChannel);
				    westboundConnReceivingList.put(newWBIChannel, newWBIConnection);
				    
				    System.out.println("Initialize WBI receiving on local port: " + newWBIConnection.getLocalInetAddress());
				}
				
				if (getWestboundChannelSending(remoteHostAddress) != null) {
					System.out.println("Sending channel already established for remote ctrl " + remoteHostAddress);
				} else {
					String localHostAddress = localAddress.getAddress().getHostAddress();
					int remotePort = localAddress.getPort();

					// int localPort = remoteAddress.getPort();
					int localPort = getNextAvailableLocalPort();

					System.out.println("Initiate WBI sending.....");
					System.out.println("localHostaddress: " + localAddress);
					System.out.println("localHostaddress: " + remoteAddress);
				    OFSwitchManager.connectSendingPort(localHostAddress, localPort, remoteHostAddress, remotePort);
				}
			}
		}
	}

	private int getNextAvailableLocalPort() {
		List<Integer> reservedPorts = new ArrayList<Integer>();
		
		if (westboundChannelSendingList.size() > 0) {
			for (String wbiChannelName : westboundChannelSendingList.keySet()) {
				Channel wbiChannel = westboundChannelSendingList.get(wbiChannelName);
				
				InetSocketAddress localAddr = (InetSocketAddress) wbiChannel.getLocalAddress();
				reservedPorts.add(localAddr.getPort());
			}
		} else {
			return 6667;
		}
		...
		int nextFreePort = reservedPorts.get(reservedPorts.size()-1)+2;
		
		return nextFreePort;
	}

	protected OFConnection createNewWBIConnection(Channel newWBIChannel) {
		DatapathId switchId = DatapathId.of(1);
		...
		OFConnection newWBIConnection = new OFConnection(switchId, factory, newWBIChannel, OFAuxId.MAIN, debugCounterService, timer);
		return newWBIConnection;
	}
	...
	public void exceptionCaught(ChannelHandlerContext ctx, ExceptionEvent e)
			throws Exception {
		// FitSDN EDIT - do not stop the WBI connection
		InetSocketAddress localAddr = (InetSocketAddress) ctx.getChannel().getLocalAddress();
		if (localAddr.getPort() >= 6666 && localAddr.getPort() <= 6669) {
			return;
		}
		...
		if (e.getMessage() instanceof List) {
			...
			for (OFMessage ofm : msglist) {
				try {
					...
					InetSocketAddress localAddress = (InetSocketAddress) e.getChannel().getLocalAddress();
					if (localAddress.getPort() == 6666) {
						if (ofm.getType() != OFType.ECHO_REQUEST && ofm.getType() != OFType.ECHO_REPLY) {
							// System.out.println("EAST-WESTBOUND RECEIVING | msg received on local port: " + e.getChannel().getLocalAddress() + " - msg: " + ofm.getType());
						}
						
						InetSocketAddress remoteAddress = (InetSocketAddress) e.getChannel().getRemoteAddress();
						String remoteHostAddress = remoteAddress.getAddress().getHostAddress();
						
						state.processOFMessageEastWestbound(ofm, remoteHostAddress);
					} else {
						if (localAddress.getPort() != 6667 && localAddress.getPort() != 6669) {
					...
					// ctx.getChannel().
					System.out.println("(OFChannelHandler) - messageReceived exception: " + ex.getMessage());
